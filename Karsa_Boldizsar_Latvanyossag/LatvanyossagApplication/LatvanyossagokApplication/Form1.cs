...
namespace LatvanyossagokApplication
{
    public partial class Form1 : Form
    {
        ...
        {
            InitializeComponent();
            //conn = new MySqlConnection("Server=localhost;Port=3307;Database=latvanyossagok_db;Uid=root;Pwd=;");
            conn = new MySqlConnection("Server=localhost;Port=3306;Database=latvanyossagok_db;Uid=root;Pwd=;");
            conn.Open();
        ...
        private void ListLatvanyossagok(int varosID)
        {
            try
            {
                ...
                cmd.CommandText = @"SELECT latvanyossagok.id, latvanyossagok.nev, leiras, ar, varos_id FROM latvanyossagok 
                                    ...
                                    WHERE latvanyossagok.id=@varos_id";
                */
                cmd.CommandText = @"SELECT latvanyossagok.id, latvanyossagok.nev, leiras, ar, varos_id FROM latvanyossagok 
                                    LEFT JOIN varosok ON latvanyossagok.varos_id = varosok.id
        ...
        private void buttonVarosSubmit_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    MessageBox.Show("Nem adta meg a lakosság számát!");
                    return;
                }
                var cmd = conn.CreateCommand();
        ...
        private void buttonLatvanyossagHozzaad_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    MessageBox.Show("Nem adta meg a leírást!");
                    return;
                }
        ...
        private void buttonVarosDelete_Click(object sender, EventArgs e)
        {
            ...
            catch(Exception ex)
            {
                ...
                {
                    MessageBox.Show("A város nem törölhető, mert tartozik hozzá látványosság!");
                    return;
                }
                MessageBox.Show("Váratlan hiba történt!");
            }
